...
const selector = createSelector(mealsFeatureSelector, state => state.meals);
...
export const allFoodSelector = createSelector(allMealsSelector, meals =>
  meals
    .map(meal => (meal.ingredients || []).map(ingredient => ingredient.food))
...
export const allPreparationStepSelector = createSelector(
  ...
  meals =>
    meals
      .map(meal => meal.preparationSteps || [])
      .reduce((arr, curr) => arr.concat(curr), [])
);

export const allCookingStepSelector = createSelector(allMealsSelector, meals =>
  meals
    .map(meal => meal.cookingSteps || [])
    .reduce((arr, curr) => arr.concat(curr), [])
);
