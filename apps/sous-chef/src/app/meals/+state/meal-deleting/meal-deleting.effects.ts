...
import { MealDeletingActionTypes, DeleteMeal, DeleteMealSuccess, DeleteMealFailure } from './meal-deleting.actions';
  ...
  @Effect() delete$ = this.actions$.pipe(
    ...
    mergeMap(action =>
      this.repository.delete(action.id).pipe(
        ...
        catchError(err => Observable.of(new DeleteMealFailure(action.id, err)))
      )
    ),
  );
