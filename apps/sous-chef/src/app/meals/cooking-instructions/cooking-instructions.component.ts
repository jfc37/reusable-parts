...
import { takeUntil, map, distinctUntilChanged, tap, filter } from 'rxjs/operators';
...
import { currentMealNameSelector, currentMealPrepWorkSelector, currentMealCookingStepsSelector } from '../+state/meals/meals.selectors';
...
export class CookingInstructionsComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.title$ = this.store.select(currentMealNameSelector).pipe(filter(Boolean));
    ...
    this.error$ = this.store.select(hasFailedLoadingAllMealsSelector).pipe(
      map(hasFailed => hasFailed && `Problem getting meal. Please try again later`)
    );
    this.prepWork$ = this.store.select(currentMealPrepWorkSelector).pipe(filter(Boolean));
    this.cookingSteps$ = this.store.select(currentMealCookingStepsSelector).pipe(filter(Boolean));
  }
