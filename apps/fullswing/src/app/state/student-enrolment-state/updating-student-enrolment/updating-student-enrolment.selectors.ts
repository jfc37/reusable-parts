...
const selector = createSelector(
  ...
  state => state.updatingStudentEnrolments,
);
...
export const currentUserEnrollingIds = createSelector(
  allUpdatingIdsSelectorFn(selector),
  ids => flatten(ids.map(id => id.split('|').splice(1))),
);
