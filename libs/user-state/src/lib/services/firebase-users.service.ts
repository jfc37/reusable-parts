...
export class FirebaseUsersService {
  ...
  public getAllUserRoles(): Observable<UserRoles[]> {
    try {
      return fromPromise(this.af.firestore.collection('user-roles/').get()).pipe(
  ...
  public getAllUsers(): Observable<User[]> {
    try {
      return fromPromise(this.af.firestore.collection('users/').get()).pipe(
        map(collection =>
  ...
  public getUser(id: string): Observable<User> {
    try {
      return fromPromise(this.af.firestore.doc(`users/${id}`).get()).pipe(
        map(doc => ({
          id: doc.id,
          email: doc.data().email,
          name: doc.data().name,
  ...
  private setUserRole(id: string, role: string, value: boolean): Observable<void> {
    try {
      return fromPromise(this.af.firestore.doc(`user-roles/${id}`).update({ [role]: value })).pipe(mapTo(null));
    } catch (e) {
      console.error('Error', e);
      return _throw(e);
    }
