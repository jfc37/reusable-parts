...
import {
  AddUserRole,
  RemoveUserRole,
} from '@reusable-parts/user-state/src/user-roles/user-roles/user-roles.actions';

@Injectable()
export class RemovingUserRolesEffects {
  @Effect()
  attempt$ = this.actions$
    .ofType<AttemptToRemoveUserRoles>(
      RemovingUserRolesActionTypes.RemoveAttempt
    )
    .pipe(
    ...
    .pipe(
      switchMap(action =>
        this.repository
          ...
          .pipe(
            mergeMap(roles => [
              new RemoveUserRole(action.id, action.role),
              new RemoveUserRolesSuccess(action.id),
            ]),
            catchError(err =>
              of(
                new RemoveUserRolesFailure(
                  action.id,
                  err || 'Failed removing user roles'
                )
              )
            )
          )
